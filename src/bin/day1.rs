...
fn process1(input: String) -> u64 {
    ...
    for line in input.lines() {
        let digits = [find_digit(line, false, false), find_digit(line, false, true)];
...
const DIGIT_SPELLED: &'static [&'static str] = &[
...
fn find_digit(line: &str, spelled: bool, rev: bool) -> char {
    ...
    let digit_value = if let Some(idx) = digit_idx { Some(String::from(line.chars().nth(idx).unwrap()).parse().unwrap()) } else { None };
    if !spelled {
    ...
    for (idx, val) in DIGIT_SPELLED.iter().enumerate() {
        ...
        if line_idx.is_some() && (spelled_idx.is_none() || rev != (spelled_idx.unwrap() > line_idx.unwrap())) {
            spelled_idx = line_idx;
            spelled_value = (idx + 1).to_string().chars().nth(0);
        }
    }

    let result = match (digit_idx, spelled_idx) {
        (Some(_), None) => digit_value.unwrap(),
        (None, Some(_)) => spelled_value.unwrap(),
        (Some(digit_idx), Some(spelled_idx)) if rev != (digit_idx < spelled_idx) => digit_value.unwrap(),
        (Some(digit_idx), Some(spelled_idx)) if rev != (digit_idx > spelled_idx) => spelled_value.unwrap(),
        _ => panic!("Nope"),
    };
